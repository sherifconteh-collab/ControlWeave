        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                {msg.role === 'assistant' ? (
                  // Render assistant messages with shared safe MarkdownContent (no dangerouslySetInnerHTML)
                  <>
                    <MarkdownContent
                      content={msg.content}
                      className="text-sm [&_strong]:font-semibold [&_h2]:text-base [&_h3]:text-sm [&_h4]:text-sm [&_pre]:text-xs [&_code]:text-xs [&_li]:mb-0.5"
                    />
                    <button
                      onClick={() => navigator.clipboard.writeText(msg.content).catch(() => {})}
                      className="mt-1.5 text-xs text-gray-400 hover:text-gray-600 border border-gray-100 rounded px-2 py-0.5 transition-colors"
                    >
                      Copy
                    </button>
                  </>
