...
const TIER_OPTIONS: Array<{
  ...
  { key: 'utilities', label: 'Utilities', description: 'Enterprise add-on. All privacy, regional, and state-level regulatory frameworks with AI-powered regulatory monitoring.', frameworkLimit: -1, isAddOn: true },
];
...
// Framework groups: all ISOs in a group count as 1 toward the tier limit
const FRAMEWORK_GROUP_LABELS: Record<string, string> = { // ip-hygiene:ignore
  iso_27000: 'ISO 27000 Series — counts as 1 framework', // ip-hygiene:ignore
  iso_ai: 'ISO AI Suite — counts as 1 framework', // ip-hygiene:ignore
};
...
const FRAMEWORK_OPTIONS: FrameworkOption[] = [
  ...
  { code: 'owasp_llm_top10', label: 'OWASP LLM Top 10 (2025)', description: 'Critical security risks for Large Language Model deployments including prompt injection and data poisoning.', controlCount: 10, tierRequired: 'professional' },
  { code: 'owasp_agentic_top10', label: 'OWASP Agentic AI Top 10 (2026)', description: 'Security risks for agentic and autonomous AI applications that act independently and chain actions.', controlCount: 10, tierRequired: 'professional' },
            ...
            <>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {TIER_OPTIONS.filter((t) => !t.isAddOn).map((tier) => (
                    <button
                      key={tier.key}
                      type="button"
                ...
                {TIER_OPTIONS.filter((t) => t.isAddOn).map((tier) => {
                  const addOnDisabled = selectedTier !== tier.key && TIER_ORDER[selectedTier] < TIER_ORDER['enterprise'];
                  return (
                  <div key={tier.key} className="mt-3">
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500 mt-1">{tier.description}</p>
                      {addOnDisabled && (
                        <p className="text-xs text-gray-400 mt-1">Select Enterprise tier first to enable this add-on.</p>
                      )}
                    </button>
                  </div>
                  );
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Select the frameworks for your organization. ISO standards within the same series are bundled and count as 1 toward your limit.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[28rem] overflow-y-auto pr-1">
                  {availableFrameworks.map((fw) => {
                    ...
                    const groupAlreadySelected = fw.group ? frameworkCodes.some((c) => FRAMEWORK_OPTIONS.find((f) => f.code === c)?.group === fw.group) : false;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2 flex items-center justify-between text-xs text-gray-500">
                          ...
                          <div className="flex items-center gap-2">
                            {fw.group && <span className="text-purple-600">{FRAMEWORK_GROUP_LABELS[fw.group] ?? fw.group}</span>}
