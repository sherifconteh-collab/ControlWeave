...
export default function OrganizationProfilePage() {
  ...
  const toggleFrameworkSelection = (frameworkId: string) => {
    ...
    if (frameworkLimit !== -1 && selectedFrameworkIds.length >= frameworkLimit) {
      const tierLabel = userTier.charAt(0).toUpperCase() + userTier.slice(1);
      setError(
        `${tierLabel} plan allows up to ${frameworkLimit} framework${frameworkLimit === 1 ? '' : 's'}. Deselect one first or upgrade tier.`
      );
      return;
    }
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-5 space-y-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <p className="text-xs text-gray-500">
              {selectedFrameworkIds.length} selected{isLimitedTier ? ` of ${frameworkLimit}` : ''}
            </p>
          </div>
          <p className="text-sm text-gray-600">
            Manage your organization frameworks here. RMF fields on this page update automatically based on what you select.
          </p>
          ...
          {availableFrameworks.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {availableFrameworks.map((framework) => {
                ...
                const isAtLimit = frameworkLimit !== -1 && selectedFrameworkIds.length >= frameworkLimit;
