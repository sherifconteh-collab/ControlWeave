...
export default function AIAnalysisPage() {
  ...
  // Swarm state
  const [swarmConfigs, setSwarmConfigs] = useState<Record<string, SwarmConfig>>({});
  const [swarmRunning, setSwarmRunning] = useState<string | null>(null);
  ...
  useEffect(() => {
    aiAPI.getSwarmConfigs()
      .then(res => setSwarmConfigs(res.data?.data || {}))
      .catch(() => {});
  ...
  const runSwarm = useCallback(async (swarmType: string) => {
    setSwarmRunning(swarmType);
    setSwarmResult(null);
    setSwarmError(null);
    try {
      const res = await aiAPI.executeSwarm({ swarmType });
      setSwarmResult(res.data?.data as SwarmResult);
      // Refresh memory stats after swarm run (new entries created)
      aiAPI.getReasoningMemoryStats()
        .then(r => setMemoryStats(r.data?.data as MemoryStats || null))
        .catch(() => {});
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: string } }; message?: string };
      setSwarmError(e.response?.data?.error || e.message || 'Swarm execution failed');
    } finally {
      setSwarmRunning(null);
    }
  }, []);
  ...
  const runFeature = useCallback(async (feature: Feature) => {
    ...
    try {
      ...
      const e = err as { response?: { data?: { error?: string } }; message?: string };
      const msg = e.response?.data?.error || e.message || 'Analysis failed. Check your AI configuration in Settings.';
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {/* ─── Multi-Agent Parallel Assessment ─────────────────────────── */}
        {Object.keys(swarmConfigs).length > 0 && (
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xl">🐝</span>
              <div>
                <h2 className="text-lg font-bold text-indigo-900">Parallel Assessment Swarms</h2>
                <p className="text-xs text-indigo-600">
                  Run multiple AI agents concurrently — gap analysis, risk scoring, audit readiness, and crosswalk optimization execute in parallel for faster results.
                </p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              {Object.entries(swarmConfigs).map(([key, config]) => (
                <button
                  key={key}
                  onClick={() => runSwarm(key)}
                  disabled={!!swarmRunning}
                  className="text-left p-3 bg-white border border-indigo-200 rounded-lg hover:border-indigo-400 hover:shadow transition-all disabled:opacity-50"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-semibold text-sm text-indigo-900">{config.name}</span>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {config.agents.map(a => (
                      <span key={a.id} className="text-xs bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">
                        {a.label}
                      </span>
                    ))}
                  </div>
                  {swarmRunning === key && (
                    <div className="mt-2 text-xs text-indigo-600 animate-pulse">⏳ Running {config.agentCount} agents in parallel…</div>
                  )}
                </button>
              ))}
            </div>
            ...
            {swarmError && (
              <div className="bg-red-100 border border-red-300 rounded-lg p-3 text-sm text-red-700 mb-3">
                {swarmError}
              </div>
            )}
            ...
            {swarmResult && (
              <div className="bg-white border border-indigo-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-indigo-900">{swarmResult.swarmName} — Results</h3>
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    {swarmResult.failureCount > 0 && (
                      <span className="text-red-600">❌ {swarmResult.failureCount} failed</span>
                    )}
                    {swarmResult.ragContextUsed && (
                      <span className="text-purple-600">📚 RAG enriched</span>
                    )}
                    {swarmResult.memoryContextUsed && (
                      <span className="text-blue-600">🧠 Memory enriched</span>
                    )}
                  </div>
                </div>
                    ...
                    <div key={agent.agentId} className={`border rounded-lg p-3 ${agent.status === 'success' ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-sm">
                          {agent.status === 'success' ? '✅' : '❌'} {agent.label}
                        </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <Link href="/dashboard/auditor-workspace"
            ...
            <div className="text-xs">
              ...
              <div className="text-indigo-600">Audit prep swarm</div>
            </div>
          </Link>
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-xl p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-blue-500">
              Memory is auto-populated when swarms, gap analysis, risk heatmaps, and other AI features run.
            </span>
          </div>
          ...
          {memoryExpanded && memoryEntries.length === 0 && !memoryLoading && (
            <div className="mt-3 text-center py-6 text-xs text-blue-400">
              No memory entries yet. Run AI analyses or swarms to populate reasoning memory.
            </div>
          )}
        </div>
          ...
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-3">
              ...
              <div className="bg-white border border-amber-200 rounded-lg p-3 text-center">
                ...
                <div className="text-xs text-amber-600">Swarm Configs</div>
              </div>
              <div className="bg-white border border-amber-200 rounded-lg p-3 text-center">
                    ...
                    <div key={feature.key} className={`rounded-xl border p-4 ${catStyle}`}>
                      ...
                      {result && (
                        <div className="mt-2">
                          ...
                          >
                            {isExpanded ? '▲ Collapse' : '▼ Show result'}
                          </button>
                          {isExpanded && (
