...
async function apiRequest(method, path, { query, body, auth = true } = {}) {
  // Validate API token is present for authenticated requests
  if (auth && !API_TOKEN) {
  ...
  return {
    ...
    handler: async (args) => {
      ...
      try {
        ...
        // 3. Log tool invocation with client metadata
...
async function main() {
  ...
  if (!API_TOKEN) {
    console.error('[WARNING] GRC_API_TOKEN not set. Most tools will fail authentication.');
    console.error('[WARNING] Set GRC_API_TOKEN environment variable with a valid JWT token.');
  }
