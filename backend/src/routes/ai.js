  ...
  return async (req, res) => {
    ...
    try {
      ...
      res.status(err.statusCode || 500).json({
        success: false,
        error: err.statusCode === 400 ? 'No AI API key configured. Add one in Settings > LLM Configuration.' : `AI ${feature} failed`
      });
    }
  };
}
...
router.post('/chat', checkAIUsage, aiHandler('chat', async (req, params) => {
  ...
  const systemPrompt = req.body.systemPrompt ||
    await llm.buildPersonalizedSystem(params.organizationId, null, 'compact', ragQuery);
...
router.post('/security-posture', checkAIUsage, aiHandler('security_posture', async (req, params) => {
  ...
  const systemPrompt = await llm.buildPersonalizedSystem(orgId, 'You are performing a comprehensive security posture analysis. Return structured JSON only.');
