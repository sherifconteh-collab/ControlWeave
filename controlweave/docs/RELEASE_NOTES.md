...
## Version 4.11.2 — September 16, 2026
...
## Version 4.11.0 — August 11, 2026

> **Version correction note**: PRs #727-739 each merged individually and the
> automation bumped PATCH each time (landing at v4.10.4); three of the four
> are feat-scope (new schema, new endpoints, new seeded catalog data) per
> `.claude/rules/git-workflow.md` and warrant MINOR. See root
> `RELEASE_NOTES.md` for the full entry.

### Added

- AU-5/AU-7/AU-11 audit controls: structured audit-pipeline failure logging,
  streaming `GET /audit/export`, and real `data_retention_policies`
  enforcement against `audit_logs`.
- NIST SP 800-53 control hierarchy and baseline scoping: `parent_control_id`/
  `is_enhancement` on `framework_controls`, a `control_baselines` table, and
  `PUT /organizations/me/baseline`.
- 714 NIST SP 800-53 Rev 5 control enhancements (300 base + 714 = 1,014 total)
  with derived FedRAMP Low/Moderate/High baselines.
- Crosswalk and assessment-procedure coverage for the 714 enhancements
  (traceability-only crosswalks, 2,931 OSCAL-derived assessment procedures).

### Fixed

- Enhancement-crosswalk invariant guard SQL operator-precedence bug.

### Security

- `audit_logs.created_at` converted to `timestamptz`; outcome integrity
  fixed.
- Per-organization SHA-384 hash chain on `audit_logs`;
  `organization_id` moves from `ON DELETE CASCADE` to `RESTRICT`.

---

## Version 4.10.4 — August 10, 2026
