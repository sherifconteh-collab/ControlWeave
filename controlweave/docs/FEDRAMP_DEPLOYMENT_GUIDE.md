...
| AU-5 | **Deployer-supplied** | No response to audit-logging failures. A failed audit write is reported to the process log and the originating request still succeeds. Alerting on audit-pipeline failure must be supplied externally. |
...
| AU-9 | Partial | Append-only is enforced in the database by the triggers in migration `121_audit_log_immutability.sql`: `DELETE` and `TRUNCATE` are rejected, and `UPDATE` is rejected for every column except `siem_forwarded`. Row-level security (`FORCE`) provides tenant isolation. This is tamper-**resistant**, not tamper-**evident** — there is no hash chain or record signature, and the application owns the table, so a privileged code path can disable the trigger. FRH-AU-9(3) requires cryptographic integrity: supplement with write-once log shipping. |
...
| AU-11 | **Deployer-supplied** | No retention period is enforced for `audit_logs`. The `data_retention_policies` table lists `audit_logs` as an intended `resource_type`, but the retention scheduler acts on evidence only. Records accumulate indefinitely, and the append-only trigger blocks deletion. |
