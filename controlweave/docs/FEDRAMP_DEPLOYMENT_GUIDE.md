...
```bash
cd controlweave/backend && node scripts/seed-frameworks.js
```
...
| `107_custom_frameworks.sql` | Custom framework builder tables | N/A |
| `108_compliance_snapshots.sql` | Compliance snapshot tracking for ConMon | CA-7 |
| `109_org_hierarchy.sql` | MSP parent-child org hierarchy | AC-2 |
...
ControlWeaver's `audit_logs` table provides part of the AU family. The table
below states what the platform implements today and what the deploying
organization must supply. Controls marked **Deployer-supplied** have no
platform implementation — do not claim them on the strength of this table.
...
| AU-8 | Implemented | `audit_logs.created_at` is `timestamptz` (migration 145), set server-side by `NOW()` and never supplied by the caller, so every record is unambiguously mappable to UTC. Time synchronization (NTP) on the database host remains deployer-supplied. |
| AU-9 | Implemented | Append-only is enforced by the triggers in migration `121`: `DELETE` and `TRUNCATE` are rejected, and `UPDATE` is rejected for every column except `siem_forwarded`. Row-level security provides tenant isolation. Records are additionally **hash-chained** (migration `147`): each row carries a SHA-384 digest of its own forensic fields plus its predecessor's digest, computed in a `BEFORE INSERT` trigger under a per-organization advisory lock. Editing or removing a record breaks every link after it. Verify with `npm run verify:audit-chain`. Records written before the chain migration carry NULL hashes and are reported as an unvouched-for pre-chain segment rather than silently passing. **FRH-AU-9(3) is satisfied for records written after the chain migration**; supplement with write-once log shipping if you need coverage of earlier history. |
| AU-10 | Partial | `user_id`, `actor_name` and `ip_address` are recorded on records written through `auditService`, and the hash chain binds each record to its position in the sequence, so a record cannot be repudiated by claiming it was inserted or reordered later. Note `user_id` is `ON DELETE SET NULL`, so deleting a user detaches the identity from historical records (`actor_name` is retained, and the hash still covers it). Records are not individually signed; FRH-AU-10 requires a digital signature — supplement with an external signing service. |
...
The ITSM connector (connector type `itsm_change`) can automatically pull incident
...
cd controlweave/backend && node scripts/snapshot-compliance.js
```
