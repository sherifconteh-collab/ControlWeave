...
ControlWeaver is a single, fully open source build — there is no separate paid
tier, no commercial Docker image, and no feature gating. Every deployment
(this repository, self-hosted or Railway-managed) runs the same code with the
same features available to every authenticated user. See
[Tier Comparison](TIER_COMPARISON.md) for background on why the old
Community/Pro/Enterprise split no longer applies.

ControlWeaver is dual-licensed: AGPL v3 (open source, free) or a commercial
...
```bash
git clone https://github.com/sherifconteh-collab/ControlWeaver-Pro.git controlweave
