...
async function findUserByEmail(client, orgId, email) {
  const result = await client.query(
    'SELECT id FROM users WHERE email_hash = $1 AND organization_id = $2',
...
async function insertPbc(client, ctx, engagementId, items) {
  ...
  for (const item of items) {
    const resolved = ['accepted', 'closed', 'rejected'].includes(item.status);
    await client.query(
      `INSERT INTO audit_pbc_requests
         (organization_id, engagement_id, title, request_details, priority, status,
          due_date, assigned_to, response_notes, created_by, resolved_at)
  ...
  for (const item of items) {
    await client.query(
      `INSERT INTO audit_signoffs
         (organization_id, engagement_id, signoff_type, status, comments, signed_by, signed_at)
...
async function run() {
  ...
  try {
    ...
    const adminId = await findUserByEmail(client, orgId, account.email);
    const auditorId = await findUserByEmail(client, orgId, `auditor@${account.industryKey}.com`);
    if (!adminId || !auditorId) {
      throw new Error(
        `Audit firm users missing (admin: ${Boolean(adminId)}, auditor: ${Boolean(auditorId)}). `
        + 'Run seed:demo-accounts and seed:auditor-accounts first.'
      );
    }

    const controlIds = await pickControlIds(client, orgId, 12);
    const ctx = { orgId, adminId, auditorId, controlIds };
