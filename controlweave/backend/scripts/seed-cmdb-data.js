  ...
  try {
    ...
    const orgResult = await client.query(`
      INSERT INTO organizations (name, tier) VALUES
        ('NovaTech Solutions', 'community'),
        ('BrightPath Health', 'pro'),
        ('Meridian Financial Group', 'enterprise'),
        ('Vanguard Defense Systems', 'govcloud')
    ...
    // Use the current shared demo tiers for CMDB sample data.
    const proOrg = orgs.find(o => o.tier === 'pro') || orgs[1];
    const communityOrg = orgs.find(o => o.tier === 'community') || orgs[0];

    // 2. Create test users
    console.log('\nCreating users...');
    const hashedPassword = await bcrypt.hash('ControlWeave!2026', 10);

    const usersResult = await client.query(`
      INSERT INTO users (organization_id, email, password_hash, first_name, last_name, role)
      VALUES
        ($1, 'admin@pro.com', $2, 'Sam', 'Rivera', 'admin'),
        ($1, 'john@pro.com', $2, 'John', 'Smith', 'user'),
        ($1, 'sarah@pro.com', $2, 'Sarah', 'Johnson', 'user'),
        ($3, 'admin@community.com', $2, 'Fred', 'Okafor', 'admin')
      ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email
      RETURNING id, email, first_name, last_name
    `, [proOrg.id, hashedPassword, communityOrg.id]);

    const users = usersResult.rows;
    console.log(`✓ ${users.length} users created`);
    console.log(`  📧 Login: admin@pro.com / ControlWeave!2026`);
    console.log(`  📧 Login: admin@community.com / ControlWeave!2026`);
    ...
    `, [proOrg.id, users[0].id, users[1].id]);
    ...
    `, [proOrg.id, categories.hardware, users[0].id, users[1].id, environments[0].id]);
    ...
    await client.query(`
      INSERT INTO assets (
        ...
        ($1, $2, 'PostgreSQL Database', '17.0', 'PostgreSQL Global Development Group',
         $3, $4, 'active',
    ...
    `, [proOrg.id, categories.software, users[0].id, environments[0].id]);
    ...
    `, [proOrg.id, categories.cloud, users[0].id, environments[0].id]);
    ...
    `, [proOrg.id, categories.network, users[0].id, environments[0].id]);
    ...
    await client.query(`
      INSERT INTO assets (
        ...
        ($1, $2, 'Customer Database - Primary', '17.0', 'PostgreSQL',
         $3, $4, 'active',
    ...
    `, [proOrg.id, categories.database, users[0].id, environments[0].id, environments[1].id]);
    ...
    `, [proOrg.id, categories.ai_agent, users[0].id, environments[0].id, environments[1].id]);
    ...
    console.log('   Pro Tier: admin@pro.com / ControlWeave!2026');
    console.log('   Community Tier: admin@community.com / ControlWeave!2026\n');
