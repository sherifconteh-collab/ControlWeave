...
let freeTierMutationApplied = false;
let freeFrameworkLimit = 2;

function normalizeTier(value) {
  return String(value || 'community').trim().toLowerCase();
}

function hasProAccess(tier) {
  return ['pro', 'professional', 'enterprise', 'govcloud', 'utilities'].includes(normalizeTier(tier));
}

function hasEnterpriseAccess(tier) {
  return ['enterprise', 'govcloud', 'utilities'].includes(normalizeTier(tier));
}
...
async function dbQuery(sql, params = []) {
  ...
  try {
    ...
    console.log(`  (DB query skipped: ${error.message})`);
...
function request(method, path, body = null, token = null, attempt = 0) {
    ...
    const req = transport.request(options, (res) => {
      ...
      res.on('end', async () => {
        ...
        if (res.statusCode === 429 && attempt < MAX_429_RETRIES) {
          ...
          await sleep(waitMs);
          resolve(await request(method, path, body, token, attempt + 1));
          return;
        }
...
async function run() {
  ...
  await test('1.3b Downgrade org to free tier for gating checks', async () => {
    ...
    const downgradeResult = await dbQuery('UPDATE organizations SET tier = $1 WHERE id = $2', ['community', adminOrgId]);
    freeTierMutationApplied = (downgradeResult.rowCount || 0) > 0;
  ...
  await test('2.1 List all frameworks', async () => {
    ...
    const minFrameworks = STRICT_DEMO_STATE ? 6 : 1;
    assert(r.body.data.length >= minFrameworks, `Expected at least ${minFrameworks} frameworks, got ${r.body.data.length}`);
  ...
  await test('2.2 Add frameworks within free-tier cap', async () => {
    const desiredIds = frameworkIds.slice(0, 2);
    let r = await request('POST', `/api/v1/organizations/${adminOrgId}/frameworks`, { frameworkIds: desiredIds }, adminToken);

    if (r.status === 200) {
      freeFrameworkLimit = desiredIds.length;
      return;
    }

    if ((STRICT_DEMO_STATE || freeTierMutationApplied) && r.status === 403) {
      const detectedLimit = Number(r.body?.maxFrameworks || 0);
      assert(Number.isFinite(detectedLimit) && detectedLimit > 0, `Expected maxFrameworks in 403 payload, got: ${JSON.stringify(r.body)}`);
      freeFrameworkLimit = detectedLimit;
      const retryIds = frameworkIds.slice(0, freeFrameworkLimit);
      r = await request('POST', `/api/v1/organizations/${adminOrgId}/frameworks`, { frameworkIds: retryIds }, adminToken);
      assert(r.status === 200, `Expected 200 after retry with ${freeFrameworkLimit} frameworks, got ${r.status}: ${JSON.stringify(r.body)}`);
      console.log(`         -> Free-tier framework cap detected as ${freeFrameworkLimit}`);
      return;
    }

    assert(false, `Expected 200, got ${r.status}: ${JSON.stringify(r.body)}`);
  });

  await test('2.3 Adding frameworks above free tier limit is blocked', async () => {
    // Org framework selection is replacement mode, so request one above detected cap.
    const r = await request('POST', `/api/v1/organizations/${adminOrgId}/frameworks`, {
      frameworkIds: frameworkIds.slice(0, Math.max(1, freeFrameworkLimit + 1))
    }, adminToken);
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      console.log(`         -> Correctly blocked above ${freeFrameworkLimit}-framework limit`);
    } else {
      assert(r.status === 200, `Expected 200, got ${r.status}: ${JSON.stringify(r.body)}`);
      console.log(`         -> ${adminOrgTier} tier can exceed the community framework cap`);
    }
  });
  ...
  await test('2.4 Dashboard stats reflect selected frameworks', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied) {
      assert(data.frameworks.length === freeFrameworkLimit, `Expected ${freeFrameworkLimit} frameworks, got ${data.frameworks.length}`);
    } else {
      assert(Array.isArray(data.frameworks), 'Frameworks breakdown should be an array');
    }
    ...
    console.log(`         -> ${data.overall.totalControls} total controls across ${freeFrameworkLimit} frameworks`);
  ...
  await test('3.4 Update control to "implemented" and check auto-crosswalk', async () => {
    ...
    const r = await request('PUT', `/api/v1/controls/${controlIds[0]}/implementation`, {
      ...
      poam_justification: 'QA automation: control implementation completed and validated.'
    }, adminToken);
    assert(r.status === 200, `Expected 200, got ${r.status}: ${JSON.stringify(r.body)}`);
  ...
  await test('3.5 Verify dashboard compliance increased', async () => {
    ...
    if (STRICT_DEMO_STATE) {
      ...
      assert(overall.implemented >= 0, 'Implemented count should be a non-negative number');
    }
  });
  ...
  await test('5.1 Evidence endpoint blocked for free tier', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      assert(r.body.requiredTier === 'pro' || r.body.minimumTier === 'pro', 'Should mention pro tier');
      console.log('         -> Evidence correctly blocked (requires Starter)');
    } else {
      assert(r.status === 200, `Expected 200, got ${r.status}`);
      assert(Array.isArray(r.body.data), 'Evidence list should be array when feature is allowed');
      console.log(`         -> Evidence available on ${adminOrgTier} tier`);
    }
  });
  ...
  await test('5.2 CMDB hardware blocked for free tier', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      assert(r.status === 200, `Expected 200, got ${r.status}`);
      assert(Array.isArray(r.body.data), 'Hardware list should be array when CMDB is allowed');
    }
  });
  ...
  await test('5.3 CMDB software blocked for free tier', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      assert(r.status === 200, `Expected 200, got ${r.status}`);
      assert(Array.isArray(r.body.data), 'Software list should be array when CMDB is allowed');
    }
  });
  ...
  await test('5.4 Reports types blocked for free tier', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      assert(r.status === 200, `Expected 200, got ${r.status}`);
      assert(Array.isArray(r.body.data), 'Report types should be array when reports are allowed');
    }
  });
  ...
  await test('5.7 Maturity score blocked for free tier (requires Professional)', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || !hasEnterpriseAccess(adminOrgTier)) {
      assert(r.status === 403, `Expected 403, got ${r.status}`);
  ...
  await test('5.8 Framework limit enforced (free = 2)', async () => {
    ...
    if (STRICT_DEMO_STATE || freeTierMutationApplied || adminOrgTier === 'community') {
      ...
      assert(r.status === 200, `Expected 200, got ${r.status}`);
      console.log(`         -> ${adminOrgTier} tier can extend framework coverage`);
    }
  });
  ...
  await test('10.8 Cross-org data access attempt', async () => {
    ...
    if (r.status === 200) {
      ...
      assert(r.status === 403 || r.status === 401 || r.status === 400, `Expected 403/401/400, got ${r.status}`);
