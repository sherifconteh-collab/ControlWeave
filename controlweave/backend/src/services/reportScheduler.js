...
async function runDueReportsSweep() {
  ...
  try {
    ...
    // already used by jobService.js's evidence_collection_rules claim) so
    // that a second process instance (PM2 cluster worker) running this same
    // sweep concurrently skips rows already locked here, instead of both
    // sending the same report email. The claim bumps next_run_at forward as
    // a placeholder; runScheduledReport overwrites it with the real
    // schedule-derived value once it completes.
