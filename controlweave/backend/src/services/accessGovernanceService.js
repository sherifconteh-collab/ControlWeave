...
/**
 * Completes an active campaign once every item is decided, generating a
 * file-less evidence record so the review itself becomes audit evidence.
 */
async function completeCampaign(orgId, campaignId, completedBy) {
  ...
  // self_reviewed counts items whose reviewer is also the subject. Wildcard
  // admins are permitted to do this (requireSod treats '*' as an authorized
  // override), so the count belongs in the evidence rather than being blocked:
  // an assessor reading this record for AC-2/AC-5 needs to know how much of the
  ...
  const { rows: [counts] } = await pool.query(
    `SELECT COUNT(*)::int AS total,
            ...
            COUNT(*) FILTER (WHERE reviewer_id IS NOT NULL
                             AND reviewer_id = subject_user_id)::int AS self_reviewed
  ...
  try {
    await client.query('BEGIN');
    const independence = counts.self_reviewed > 0
      ? `${counts.self_reviewed} of these item(s) were decided by the subject themselves under an administrator override, `
        + `so ${counts.total - counts.self_reviewed} were independently reviewed. `
      : 'Every item was decided by someone other than its subject. ';
    ...
    const { rows: [evidence] } = await client.query(
      `INSERT INTO evidence (organization_id, uploaded_by, description, tags)
       VALUES ($1, $2, $3, ARRAY['access-review', 'ac-2']) RETURNING id`,
      [orgId, completedBy, description]
    );
