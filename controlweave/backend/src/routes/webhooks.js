...
router.post('/', async (req, res) => {
  try {
    ...
    const status = String(error.message || '').includes('target_url') ? 400 : 500;
    res.status(status).json({
      success: false,
      error: status === 400 ? error.message : 'Failed to create webhook subscription'
    });
  }
});
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    res.status(status).json({
      success: false,
      error: status === 400 ? error.message : 'Failed to update webhook subscription'
    });
  }
});
