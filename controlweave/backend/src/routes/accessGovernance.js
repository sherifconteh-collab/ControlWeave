 ...
 * access testing), and access review certification campaigns.
 */
...
// CodeQL can trace a recognized rate-limiting middleware covering every
// route below — the Redis-backed org-scoped limiter beneath remains the real
// production control. Matches the pattern in audit.js / trustCenter.js.
router.use(rateLimit({ windowMs: 15 * 60 * 1000, max: 300 }));

router.use(authenticate);

// Rate limiter: 120 requests per 15 minutes per org (matches TPRM router policy)
const accessGovernanceRateLimiter = createRateLimiter({
...
const VALID_DOCUMENT_EXTENSIONS = ['.pdf', '.doc', '.docx', '.txt', '.md', '.csv'];

// Uploads are processed entirely in memory: only the extracted text is stored
// (rbac_documents.extracted_text) and the original file is discarded, so there
// is no file-serving surface for these documents.
const documentUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 10 * 1024 * 1024 },
  fileFilter: (req, file, cb) => {
    if (!VALID_DOCUMENT_EXTENSIONS.includes(path.extname(file.originalname || '').toLowerCase())) {
      const err = new Error(`Unsupported file type. Supported: ${VALID_DOCUMENT_EXTENSIONS.join(', ')}`);
      err.status = 400;
      return cb(err);
    }
    return cb(null, true);
...
// (the /ai/rbac-analysis endpoint returns the structured result; saving it is
// an explicit human action, consistent with human-review-before-acceptance)
...
// would fall through to 500 for an oversize file. Mirrors routes/evidence.js.
