...
// IP-based bound is in place before any DB/JWT work runs. The per-route limits
// below remain the real control. Same pattern as accessGovernance.js.
...
router.get('/activity/feed', requirePermission('implementations.read'), async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN control_implementations ci_legacy ON ci_legacy.id = al.resource_id AND ci_legacy.organization_id = al.organization_id
...
router.get('/:id', requirePermission('implementations.read'), async (req, res) => {
  try {
    ...
    implementation.assigned_to_email = implementation.assigned_to_email
      ? decryptEmailSafe(implementation.assigned_to_email)
      : implementation.assigned_to_email;
...
router.patch('/:id/status',
  ...
  try {
    ...
    // control_implementations id) to stay consistent with controls.js
    await pool.query(
    ...
    if (status === 'verified') {
      ...
      const frameworkControlId = ctrl.rows[0]?.framework_control_id || null;
      ...
      await createNotification(
        ...
        frameworkControlId ? `/dashboard/controls/${frameworkControlId}` : null
      );
    }
