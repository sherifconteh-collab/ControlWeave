...
async function fetchControlRows(orgId, specificControlId = null) {
  ...
  const result = await pool.query(
    `SELECT
       ...
       ev.last_evidence_at,
       asmt.last_assessed_at,
       asmt.last_assessment_status,
       COALESCE(vwx.open_control_impacts, 0) AS open_control_impacts,
       COALESCE(vwx.non_compliant_impacts, 0) AS non_compliant_impacts,
       COALESCE(px.open_poam_items, 0) AS open_poam_items,
       cex.has_active_exception
