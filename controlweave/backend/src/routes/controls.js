...
// per-route caps below so those stay the binding constraint in normal use;
// this layer only catches broad abuse. Same pattern as accessGovernance.js.
...
router.get('/:id',
  ...
  try {
    ...
    // When a control is credited via crosswalk rather than its own evidence, an
    // assessor's first question is which control it was credited from. Return
    // the provenance alongside the status so the answer is in the response
    // rather than implied.
    ...
    await pool.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          crosswalkCreditsApplied: appliedCredits.length,
