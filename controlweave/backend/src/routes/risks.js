 ...
 *
 * What existed before was `risk_scores` — one computed posture number per
 * organization — which is a metric, not a register. ControlWeaver shipped
 * ISO 31000 and ISO 27005 as assessable frameworks with nowhere to record the
 * risks they are about.
 */
...
// rather than on the evidence. Mirrors the CHECK constraint in migration 143.
...
router.get('/:id', requirePermission('risks.read'), async (req, res) => {
  try {
    ...
    const [treatments, controls, assets, objectives, reviews, poams, vendors, evidence] = await Promise.all([
      ...
      // Third parties, added by migration 142. risk_tier is the vendor's static
      ...
      pool.query(
        // The column is vendor_name (migration 072); aliased to `name` so the
        // response shape matches what the risk detail page already renders.
      ...
      // Evidence, added by migration 143. pii_classification travels with the
      ...
      pool.query(
        `SELECT rel.id, rel.evidence_id, rel.relevance, rel.notes,
                ...
                e.pii_classification, e.expires_at, e.created_at AS uploaded_at
...
const LINK_KINDS = {
  ...
  // impact and a treatment. Added by migration 142 so vendor concentration is
  ...
  // Evidence, added by migration 143 -- the last unconnected edge. Evidence
  // could already be linked to controls (migration 009), so a risk's evidence
  // was only reachable transitively: via its controls, and only when those
  // controls happened to have evidence. "Show me this risk is under
