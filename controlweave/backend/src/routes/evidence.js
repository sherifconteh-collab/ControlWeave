...
//
// The reverse of POST /risks/:id/evidence (migration 143). Evidence could
...
router.get('/:id/risks',
  ...
  try {
    ...
    console.error('Evidence risks error:', error);
...
router.post('/:id/link',
  ...
  try {
    ...
    for (const cid of controlIds) {
      await pool.query(
        'INSERT INTO evidence_control_links (evidence_id, control_id, notes, organization_id) SELECT $1, $2, $3, e.organization_id FROM evidence e WHERE e.id = $1 ON CONFLICT DO NOTHING',
