...
if (process.env.SENTRY_DSN) {
  ...
  require('./utils/logger').setSentryClient(_sentry);
}
...
(function validateRequiredEnv() {
  if (!databaseConfigured) {
    const missingIndividual = Array.isArray(pool.missingConfig) ? pool.missingConfig : [];
  ...
  if (!databaseConfigured) {
    ...
    const health = {
      ...
      database: {
        ...
        missingConfig: Array.isArray(pool.missingConfig) ? pool.missingConfig : []
      },
      memory: {
...
const customFrameworksRoutes = require('./routes/customFrameworks');
...
app.use('/api/v1/frameworks/custom', customFrameworksRoutes);
...
app.use('/api/v1/reports/scheduled', scheduledReportsRoutes);
...
app.use((err, req, res, next) => {
  ...
  // Only forward the original message for explicit client errors (4xx).
  // For server errors (5xx) return a generic message to prevent leaking
  // internal details such as database connection strings or file paths.
...
async function ensurePlatformAdminSelfAssessment() {
  const email = String(process.env.PLATFORM_ADMIN_EMAIL || '').trim().toLowerCase();
  if (!email) return;
  ...
  const status = await pool.query(
    `SELECT
        u.organization_id,
        (SELECT COUNT(*)::int FROM frameworks) AS framework_count,
        (SELECT COUNT(*)::int FROM framework_controls) AS framework_control_count,
        (SELECT COUNT(*)::int FROM control_implementations ci WHERE ci.organization_id = u.organization_id) AS impl_count
     FROM users u
  ...
  if (Number(row.impl_count || 0) > 0) {
    log('info', 'platform.admin.self_assessment_present', {
      email,
      organizationId: row.organization_id,
      implementationCount: Number(row.impl_count || 0)
    });
    return;
  }

  if (Number(row.framework_count || 0) === 0 || Number(row.framework_control_count || 0) === 0) {
    log('warn', 'platform.admin.self_assessment_skipped', {
      email,
      organizationId: row.organization_id,
      reason: 'framework_catalog_missing'
    });
    return;
  }

  const { spawn } = require('child_process');
  const scriptPath = path.join(__dirname, '../scripts/seed-controlweave-self-assessment.js');
  log('info', 'platform.admin.self_assessment_seeding', { email, organizationId: row.organization_id });
  const child = spawn(process.execPath, [scriptPath], { env: process.env, stdio: 'inherit' });
  await new Promise((resolve, reject) => {
    child.on('close', (code) => code === 0 ? resolve() : reject(new Error(`Self-assessment seed exited with code ${code}`)));
...
async function ensureDemoAccountsSeeded() {
  ...
  const demoEmails = DEMO_ADMIN_ACCOUNTS
  ...
  for (const email of demoEmails) {
    ...
    if (Number(row.impl_count || 0) === 0) {
      emptyAccounts.push(email);
    }
  }

  if (missingAccounts.length === 0 && emptyAccounts.length === 0) {
    log('info', 'demo.seed.present', { accounts: demoEmails.length });
    return;
  }

  log('info', 'demo.seed.starting', {
    missingAccounts,
    emptyAccounts
  });

  const { spawn } = require('child_process');
  const scriptPath = path.join(__dirname, '../scripts/seed-demo-accounts-full-test-data.js');
  const child = spawn(process.execPath, [scriptPath], { env: process.env, stdio: 'inherit' });
  await new Promise((resolve, reject) => {
    child.on('close', (code) => code === 0 ? resolve() : reject(new Error(`Demo seed exited with code ${code}`)));
    child.on('error', reject);
  });
}
...
async function ensureLicenseFromDb() {
  ...
  if (!activeKey && databaseConfigured) {
  ...
  if (!activeKey && !databaseConfigured) {
    log('info', 'license.db_lookup_skipped', {
    ...
    const server = app.listen(PORT, HOST, () => {
      ...
      if (databaseConfigured) {
        ...
        ensureAssessmentProcedures()
          ...
          .catch((err) =>
            log('error', 'startup.seed_error', { error: err.message })
          );
      } else {
        log('info', 'server.startup.db_tasks_skipped', {
          reason: 'database_not_configured'
        });
      }
    });
