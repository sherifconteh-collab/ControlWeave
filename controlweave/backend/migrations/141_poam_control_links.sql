-- Migration 141: Many-to-many POA&M <-> control linkage
--
...
--   * risks     -> controls  via `risk_control_links` (migration 136)
...
--
-- Ships alongside migration 140 in the remediation-integration release.
...
SELECT p.organization_id, p.id, p.control_id,
       'Backfilled from poam_items.control_id (migration 141)', p.created_by
FROM poam_items p
