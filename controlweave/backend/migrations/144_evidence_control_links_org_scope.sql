-- =============================================================================
-- 144_evidence_control_links_org_scope.sql
--
-- Gives evidence_control_links the organization_id every other link table in
-- this schema already carries. Flagged in migration 143's header as the weaker
...
-- link tables (136, 140, 141, 142, 143) all carry organization_id with a
...
--
-- In this repository the table is created exactly once, in migration 009, with
-- a composite primary key (evidence_id, control_id) and no surrogate id. That
-- is the live shape, verified against a database built by running the full
-- migration set rather than inferred from reading the files.
--
-- An earlier revision of this header claimed migration 014 re-declared the
-- table and was shadowed into a no-op. That is true of the ai-grc mirror, whose
-- 014 is 014_evidence_management.sql; this repository's 014 is
-- 014_remove_nist_800_160_framework.sql and does not mention the table. The
-- claim was carried across from the mirror without being re-checked here --
-- exactly the blind port this pair of changes exists to prevent.
--
-- The shape also differs slightly from the mirror, which carries a `linked_by`
-- column this repo does not. Checked rather than assumed.
--
-- BACKFILL
--
