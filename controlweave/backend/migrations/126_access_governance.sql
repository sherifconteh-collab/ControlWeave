-- Migration 126: Access governance (SoD rules, access review campaigns, entitlement snapshots)
--
-- Why: ControlWeaver had no in-app access governance -- no toxic-combination
...
--      campaign generates an evidence record (file-less) as audit evidence.
--   3. Seeds the access_governance.read / access_governance.manage permissions
--      (same bug class as migrations 116/117/119: every requirePermission()
--      key must be seeded or non-admins get a blanket 403).
-- Ships with the access governance module (v4.8.0).
