-- Migration 136: Risk register
--
-- ControlWeaver ships ISO 31000, ISO 27005 and the NIST AI RMF as frameworks a
-- customer can assess against, but had nowhere to record the risks those
-- frameworks are about. The only risk-shaped table was `risk_scores` (migration
-- 057): a single computed 0-100 posture number per organization, recalculated
-- from control coverage and vulnerability counts. That is a *score*, not a
-- register. There was no way to record an individual risk, assess it, assign an
-- owner, decide a treatment, or show an assessor the trail from identification
-- to acceptance.
--
-- This migration adds the register. It is deliberately modeled on ISO 31000 /
...
-- than parallel to it.
--
-- Ships in the risk and resilience release alongside migrations 135, 137-139.
