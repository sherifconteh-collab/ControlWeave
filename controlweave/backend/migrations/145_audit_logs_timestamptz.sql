-- Migration 145: Convert audit_logs.created_at to timestamptz (NIST AU-8)
--
...
--
-- Note on the append-only triggers from migration 121: ALTER TABLE ... ALTER
-- COLUMN TYPE performs a table rewrite rather than row-level DML, so the
-- BEFORE UPDATE trigger audit_logs_no_update does not fire and does not need
-- to be disabled for this migration.
--
...
SELECT 'Migration 145 completed.' AS result;
