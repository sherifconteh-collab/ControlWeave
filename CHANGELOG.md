...
1. Keep all in-progress changes under `[Unreleased]`.
2. Cut a release branch as `release/CW-<major>.<minor>.<patch>`.
3. Move `[Unreleased]` entries into a new `## [<major>.<minor>.<patch>] — YYYY-MM-DD` section.
4. Create and push a git tag `v<major>.<minor>.<patch>` to trigger `.github/workflows/release.yml`.
...
1. Append new entries under `[Unreleased]` in the appropriate category
2. On release, move `[Unreleased]` items to a new versioned section with today's date
3. Suggest a conventional commit message referencing the changelog update
4. Flag any breaking changes with a `⚠️ BREAKING` label in the entry
