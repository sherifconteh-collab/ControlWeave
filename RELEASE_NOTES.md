...
## v4.11.0 -- August 11, 2026

> **Release Date**: 2026-08-11
> **Version**: 4.11.0
>
> **Version correction note**: the four PRs below (#727-739) each merged
> individually and the automation bumped PATCH each time (landing at
> v4.10.4), because its merge-subject scan runs per-push against a range that
> didn't consistently pick up the `feat:` merge commits among them. Three of
> the four are genuinely feat-scope per `.claude/rules/git-workflow.md`
> (new schema, new endpoints, new seeded catalog data) and warrant a MINOR
> bump. This entry corrects that rather than leaving it under `v4.10.x`.

### Added

- **AU-5/AU-7/AU-11 audit controls** (#728) — structured failure logging for
  the audit pipeline itself (previously `console.error` and silently
  swallowed), a streaming `GET /audit/export` (CSV/JSON) endpoint, and real
  enforcement of `data_retention_policies` rows against `audit_logs` with a
  365-day-default, 90-day-floor retention window.
- **NIST SP 800-53 control hierarchy and baseline scoping** (#733) — adds
  `parent_control_id`/`is_enhancement` to `framework_controls` and a
  `control_baselines` table recording NIST SP 800-53B Low/Moderate/High
  membership per control; a new `PUT /organizations/me/baseline` endpoint lets
  an org scope its compliance percentage to the baseline it's actually
  pursuing instead of the full catalog.
- **714 NIST SP 800-53 Rev 5 control enhancements** (#735) — the catalog goes
  from 300 base-only controls to the full 1,014-item Rev 5.2.0 catalog (300
  base + 714 non-withdrawn enhancements), each carrying its parent link and
  baseline membership; FedRAMP Low/Moderate/High are now derived from the
  same profile data rather than hand-maintained.
- **Crosswalk and assessment-procedure coverage for the 714 enhancements**
  (#739) — intra-catalog and inherited crosswalk mappings (traceability only,
  `mapping_type='related'`, never auto-crediting compliance) and 2,931
  NIST SP 800-53A assessment procedures extracted directly from the OSCAL
  catalog.

### Fixed

- **Enhancement-crosswalk invariant guard** — a SQL operator-precedence bug
  (`WHERE A OR B AND C` parses as `WHERE A OR (B AND C)`) made the guard that
  refuses to auto-credit enhancement mappings fire on every safe intra-catalog
  row. Caught while porting the script to the ai-grc-platform mirror; fixed
  here in the same PR that introduced it.

### Security

- **`audit_logs.created_at` converted to `timestamptz`; outcome integrity
  fixed** (#727) — `success` is now derived from the validated `outcome`
  field instead of hardcoded `true`.
- **Per-organization SHA-384 hash chain on `audit_logs`** (#732) — makes the
  append-only enforcement added earlier in this series tamper-*evident*, not
  just tamper-resistant; `audit_logs.organization_id` moves from
  `ON DELETE CASCADE` to `RESTRICT`.

---

## v4.10.4 -- August 10, 2026
